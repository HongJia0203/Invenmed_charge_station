...
void APP_interruptOccurredAtEX1(void) {
    ...
    switch (stSystemInfo.stInterruptInfo.u8Emergency_Stop_Flow) {
        ...
        {
            DebugAndRfid_Printf("Occur Interrupt\r\n");
...
void getCPPinValue(void) {
    ...
    printf("HERE is CP is %d\r\n", adc2_result);
        }
    ADC2_Disable();
//     根據測量結果判斷狀態
        if (CP_VEHICLE_CONNECTED(adc2_result))
            stSystemInfo.stChargeInfo.CPs[1]++;
        else if (CP_READY(adc2_result))
            stSystemInfo.stChargeInfo.CPs[2]++;
        else if (CP_READY_VENTILATION(adc2_result))
            stSystemInfo.stChargeInfo.CPs[3]++;
